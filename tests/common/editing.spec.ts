...
["community", "pro"].forEach((edition) => {
  test.describe(`${edition} editing tests`, () => {
    const url = edition === "pro" ? "editing/pro" : "editing";

    test.beforeEach(async ({ page }) => {
      await page.goto(url);
    });

    test(`${edition} deleting a row works as expected`, async ({ page }) => {
    ...
    test(`${edition} editing a row works as expected`, async ({ page }) => {
      const editableGridContainer = page.getByTestId("editable grid container");
      ...
      const strInput = secondRow.getByRole("textbox", {
        name: "String Column",
      });
      await strInput.fill("Edited Row");
      const numInput = secondRow.getByRole("spinbutton", {
        name: "Number Column",
      });
