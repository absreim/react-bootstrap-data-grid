...
file to a directory that is convenient for you to reference in another project, like your home directory.

In the project with you want to test the tarball, install it with NPM with a command like the following. Be sure to
change the path to the actual path that you placed the tarball.

```bash
npm install ~/absreim-react-bootstrap-data-grid-1.4.1.tgz
```

One successfully installed, the dependency entry in the `package.json` file will be updated to reflect the file's
location. For example, the line in `package.json` may look as follows:

```
"@absreim/react-bootstrap-data-grid": "file:../../absreim-react-bootstrap-data-grid-1.4.1.tgz"
```

Be sure to change the line back point to a version number once your testing is done
