...
import { ColDataTypeStrings, ColDef, ColSortModel, RowDef, Size, TableSortModel } from "./types";
...
const getTypeComparator: (typeStr: ColDataTypeStrings) => (a: any, b: any) => number = (typeStr) => {
...
const getRowComparator: (comparator: (a: any, b: any) => number, fieldName: string) => (rowA: RowDef, rowB: RowDef) => number = (comparator, fieldName) => {
        ...
        <thead>
          <tr>
            {cols.map(({ name, label, sortable }) => {
              const colSortModel: ColSortModel | undefined = sortModel && sortable ? ({
