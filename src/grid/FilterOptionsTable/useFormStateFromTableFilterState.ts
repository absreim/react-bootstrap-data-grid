import { DateFilterState, TableFilterState } from "../types";
import { FilterFormState } from "./types";
import { useMemo } from "react";
import { dateToDatetimeInputStr, dateToInputStr } from "../util/datetime";

const useFormStateFromTableFilterState: (tableFilterState: TableFilterState) => FilterFormState = (tableFilterState) => {
  return useMemo(() => Object.keys(tableFilterState).reduce((filterFormState, colName) => {
    ...
    switch (editableState.type) {
      ...
      default: { // date or datetime
        ...
        const dateToStrConverter: (date: Date) => string = editableState.type === "date" ? dateToInputStr : dateToDatetimeInputStr
  ...
  }, {} as FilterFormState), [tableFilterState])
}
